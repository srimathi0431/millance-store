...
const SizeVariantBuilder: React.FC<{
  productSku: string; productPrice: string; editVariants: any[];
...
}> = ({ productSku, productPrice, editVariants, onAddVariants, onUpdateVariant, onDeleteVariant, showToast }) => {
  const [selectedSizes, setSelectedSizes] = React.useState<string[]>([]);
  const [color, setColor] = React.useState('');
  const [sizeRows, setSizeRows] = React.useState<Record<string, { stock: string; sku: string; price: string }>>({});
  const [mode, setMode] = React.useState<'sizes'|'custom'>('sizes');
  ...
  const fmtV = (n: number) => new Intl.NumberFormat('en-IN',{style:'currency',currency:'INR',maximumFractionDigits:0}).format(n);
                      ...
                      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8 }}>
                        {[['Stock','stock','number'],['Price ₹','price','number'],['SKU','sku','text']].map(([lbl,key,type])=>(
                          <div key={key}>
                            ...
                            <input className="vendor-form-input" style={{ padding:'6px 8px', fontSize:13 }} type={type} min={type==='number'?'0':'undefined'} value={(row as any)[key]} onChange={e=>setSizeRows(prev=>({ ...prev, [size]: { ...prev[size], [key]: type==='text'?e.target.value.toUpperCase():e.target.value } }))} />
                          </div>
                        ))}
                      </div>
                    </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
            {[['Name','name','text','e.g. 256GB Black'],['SKU *','sku','text','e.g. PROD-256-BK'],['Price (₹) *','price','number',''],['Stock','stock','number',''],['Color','color','text','e.g. Red']].map(([lbl,key,type,ph])=>(
              <div className="vendor-form-group" key={key}>
                ...
                <input className="vendor-form-input" type={type} min={type==='number'?'0':'undefined'} placeholder={ph} value={(customForm as any)[key]} onChange={e=>setCustomForm(p=>({...p,[key]:key==='sku'?e.target.value.toUpperCase():e.target.value}))} />
              </div>
            ))}
...
const VendorProducts: React.FC = () => {
  ...
  const catMap: Record<number, string> = Object.fromEntries(allCats.map(c => [c.id, c.name]));

  // ── Mutations ────────────────────────────────────────
  const createMut = useMutation({
    mutationFn: (d: any) => vendorProductsApi.create(d),
    onSuccess: async (res) => {
    ...
    onError: (e: any) => showToast('error', e?.response?.data?.message || 'Failed to create product'),
  });

  const updateMut = useMutation({
    mutationFn: ({ id, d }: any) => vendorProductsApi.update(id, d),
    onSuccess: () => { qc.invalidateQueries(['vendor-products']); setShowModal(false); showToast('success', 'Product updated'); },
    onError: (e: any) => showToast('error', e?.response?.data?.message || 'Failed to update'),
  });

  const statusMut = useMutation({
    mutationFn: ({ id, s }: any) => vendorProductsApi.updateStatus(id, s),
    onSuccess: () => { qc.invalidateQueries(['vendor-products']); showToast('success', 'Status updated'); },
    onError: (e: any) => showToast('error', e?.response?.data?.message || 'Failed'),
  });

  const addImageMut = useMutation({
    mutationFn: ({ pid, data }: any) => vendorProductsApi.addImage(pid, data),
    onSuccess: async () => { await refreshEdit(); showToast('success', 'Image added'); },
    onError: (e: any) => showToast('error', e?.response?.data?.message || 'Failed to add image'),
  });

  const delImageMut = useMutation({
    mutationFn: ({ pid, iid }: any) => vendorProductsApi.deleteImage(pid, iid),
    onSuccess: async () => { await refreshEdit(); showToast('success', 'Image removed'); },
    onError: (e: any) => showToast('error', e?.response?.data?.message || 'Failed'),
  });

  const primaryImageMut = useMutation({
    mutationFn: ({ pid, iid }: any) => vendorProductsApi.setPrimaryImage(pid, iid),
    onSuccess: async () => { await refreshEdit(); },
  });

  const createVariantMut = useMutation({
    mutationFn: ({ pid, data }: any) => vendorProductsApi.createVariant(pid, data),
    onSuccess: async () => {
      await refreshEdit();
      setVariantForm(emptyVariant()); setShowVariantForm(false); setEditingVariantIdx(null);
      showToast('success', 'Variant added');
    },
    onError: (e: any) => showToast('error', e?.response?.data?.message || 'Failed to add variant'),
  });

  const updateVariantMut = useMutation({
    mutationFn: ({ pid, vid, data }: any) => vendorProductsApi.updateVariant(pid, vid, data),
    onSuccess: async () => {
      await refreshEdit();
      setVariantForm(emptyVariant()); setShowVariantForm(false); setEditingVariantIdx(null);
  ...
  const delVariantMut = useMutation({
    mutationFn: ({ pid, vid }: any) => vendorProductsApi.deleteVariant(pid, vid),
                        ...
                        {p.variants?.length > 0 ? (
                          <div>
                            ...
                            <span style={{ marginLeft: 4, fontSize: 11, color: p.variants.some((v: any) => v.stock > 0) ? '#059669' : '#DC2626' }}>
                              {p.variants.some((v: any) => v.stock > 0) ? '● Avail' : '● OOS'}
                            </span>
                          </div>
                      ...
                      {editImages.map((img: any, idx: number) => (
                        <div key={img.id || idx} style={{ borderRadius: 10, overflow: 'hidden', border: img.is_primary ? '2px solid #7B2FF7' : '1px solid #E2E8F0', position: 'relative' }}>
                          <img src={img.image_url} alt="" style={{ width: '100%', height: 110, objectFit: 'cover', display: 'block' }}
                            onError={e => { (e.target as HTMLImageElement).src = 'https://placehold.co/120x110/F1F5F9/94A3B8?text=Error'; }} />
                          {img.is_primary && (
