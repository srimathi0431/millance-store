...
type ReportTab = 'sales'|'orders'|'products'|'customers';
...
const ReportsPage: React.FC = () => {
  ...
  const TABS:ReportTab[] = ['sales','orders','products','customers'];
          ...
          <>
            ...
            {tab==='sales' && (!salesData||!salesData.length ? (
              ...
              <table className="admin-table">
                ...
                <tbody>{salesData.map((r:any,i:number)=><tr key={i}><td style={{color:'#64748B'}}>{String(r.period).substring(0,16)}</td><td style={{fontWeight:600}}>{r.transactions}</td><td style={{fontWeight:700,color:'#FF7A00'}}>{fmt(r.revenue)}</td></tr>)}</tbody>
              </table>
            ))}
            ...
            {tab==='orders' && (
              <div style={{padding:20}}>
                ...
                : <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:14}}>
                  {Object.entries(ordersData).map(([status,count]:any)=>(
            ...
            {tab==='products' && (!productsData||!productsData.length ? (
              ...
              <table className="admin-table">
                ...
                <tbody>{productsData.map((p:any,i:number)=><tr key={i}><td style={{color:'#94A3B8',fontWeight:700}}>{i+1}</td><td style={{fontWeight:600,maxWidth:240,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{p.product_name}</td><td>{p.total_sold}</td><td style={{fontWeight:700,color:'#FF7A00'}}>{fmt(p.total_revenue)}</td></tr>)}</tbody>
              </table>
            ))}
