...
const statusColor = (s:string) => ({PENDING:'warning',CONFIRMED:'info',PROCESSING:'purple',PACKED:'info',SHIPPED:'orange',OUT_FOR_DELIVERY:'orange',DELIVERED:'success',CANCELLED:'danger',RETURN_REQUESTED:'warning',RETURNED:'gray'}[s]||'gray');
...
const OrdersPage: React.FC = () => {
  ...
  const statusMut = useMutation({
    mutationFn: ({ id, s }: any) => ordersApi.updateStatus(id, s),
    onSuccess: () => { qc.invalidateQueries(['admin-orders']); qc.invalidateQueries(['admin-order']); toast('success','Status updated'); },
    onError: (e:any) => toast('error', e?.response?.data?.message || 'Invalid transition'),
  });

  const cancelMut = useMutation({
    mutationFn: ({ id, reason }: any) => ordersApi.cancel(id, reason),
