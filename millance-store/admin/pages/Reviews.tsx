...
const statusColor = (s:string) => ({PENDING:'warning',APPROVED:'success',REJECTED:'danger',HIDDEN:'gray'}[s]||'gray');
...
const ReviewsPage: React.FC = () => {
  ...
  const statusMut = useMutation({ mutationFn:({id,s}:any)=>reviewsApi.updateStatus(id,s), onSuccess:()=>{qc.invalidateQueries(['admin-reviews']);toast('success','Status updated');}, onError:(e:any)=>toast('error',e?.response?.data?.message||'Failed') });
  const deleteMut = useMutation({ mutationFn:(id:number)=>reviewsApi.delete(id), onSuccess:()=>{qc.invalidateQueries(['admin-reviews']);setDeleteId(null);toast('success','Review deleted');} });
