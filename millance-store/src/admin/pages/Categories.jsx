...
const CategoriesPage= () => {
 ...
 onError: (e) => toast('error', e?.response?.data?.message || 'Cannot delete — subcategories or products depend on it'),
 });
 const statusMut = useMutation({
 mutationFn: ({ id, v }) => categoriesApi.updateStatus(id, v),
 onSuccess: () => { qc.invalidateQueries(['admin-categories-all']); toast('success', 'Status updated'); },
 });

 const openCreate = () => { setEditing(null); setForm(emptyForm); setShowModal(true); };
 const openEdit = (c) => { setEditing(c); setForm({ name: c.name, description: c.description || '', sort_order: String(c.sort_order || 0), is_active: c.is_active }); setShowModal(true); };
 ...
 const colorFor = (idx) => CAT_COLORS[idx % CAT_COLORS.length];
 const iconFor = (name) => CAT_ICONS[name] || '📦';
 ...
 onMouseEnter={e => { const el = e.currentTarget; el.style.transform = 'translateY(-4px)'; el.style.boxShadow = `0 12px 32px rgba(15,23,42,0.12)`; el.style.borderColor = color; }}
 onMouseLeave={e => { const el = e.currentTarget; el.style.transform = 'translateY(0)'; el.style.boxShadow = '0 1px 4px rgba(15,23,42,0.05)'; el.style.borderColor = '#E2E8F0'; }}
