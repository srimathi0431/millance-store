...
const SizeVariantBuilder = ({ productSku, productPrice, editVariants, onAddVariants, onUpdateVariant, onDeleteVariant, toast }) => {
 const [mode, setMode] = useState('sizes');
...
const VariantRow = ({ v, idx, attrs, onUpdate, onDelete }) => {
 const [editStock, setEditStock] = useState(String(v.stock));
 const fmt2 = (n) => new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 }).format(n);
 return (
 <tr>
 <td>
...
const ProductsPage= () => {
 ...
 mutationFn: ({ id, d }) => productsApi.update(id, d),
 ...
 mutationFn: ({ pid, vid, data }) => productsApi.updateVariant(pid, vid, data),
 onSuccess: () => { fetchEditProduct(editing?.id); },
 onError: (e) => toast('error', e?.response?.data?.message || 'Failed'),
 });

 const delVariantMut = useMutation({
 mutationFn: ({ pid, vid }) => productsApi.deleteVariant(pid, vid),
 ...
 onError={e => { (e.target).src = 'https://placehold.co/130x120/F1F5F9/94A3B8?text=Error'; }} />
 {img.is_primary && (
