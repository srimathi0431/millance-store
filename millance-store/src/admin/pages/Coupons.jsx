...
const CouponsPage= () => {
 ...
 const createMut = useMutation({ mutationFn: (d)=>couponsApi.create(d), onSuccess:()=>{qc.invalidateQueries(['admin-coupons']);setShowModal(false);toast('success','Coupon created');}, onError:(e)=>toast('error',e?.response?.data?.message||'Failed') });
 const updateMut = useMutation({ mutationFn:({id,d}) =>couponsApi.update(id,d), onSuccess:()=>{qc.invalidateQueries(['admin-coupons']);setShowModal(false);toast('success','Updated');}, onError:(e)=>toast('error',e?.response?.data?.message||'Failed') });
 const deleteMut = useMutation({ mutationFn:(id)=>couponsApi.delete(id), onSuccess:()=>{qc.invalidateQueries(['admin-coupons']);setDeleteId(null);toast('success','Deleted');} });
 const statusMut = useMutation({ mutationFn:({id,v}) =>couponsApi.updateStatus(id,v), onSuccess:()=>{qc.invalidateQueries(['admin-coupons']);toast('success','Status updated');} });
