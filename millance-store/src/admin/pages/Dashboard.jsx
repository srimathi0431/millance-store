...
const DashboardPage= () => {
 ...
 const kpis = dash ? [
 { label: 'Total Revenue', value: fmt(dash.revenue?.total_revenue ?? 0), icon: DollarSign, color: '#FF7A00', bg: '#FFF7ED' },
 { label: 'Total Orders', value: fmtNum(dash.orders?.total_orders ?? 0), icon: ShoppingCart, color: '#7B2FF7', bg: '#F5F3FF' },
 { label: 'Total Products', value: fmtNum(dash.products?.total_products ?? 0), icon: Package, color: '#FF3D8D', bg: '#FFF1F8' },
 { label: 'Total Customers', value: fmtNum(dash.customers?.total_customers ?? 0), icon: Users, color: '#10B981', bg: '#ECFDF5' },
 ] : [];
 ...
 <div>
 {salesData.slice(-8).map((s, i) => {
 const maxRev = Math.max(...salesData.map((x) => x.revenue));
 const pct = maxRev > 0 ? (s.revenue / maxRev) * 100 : 0;
 const label = s.date || s.week || s.month || `Item ${i+1}`;
 return (
