...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  void didChangeDependencies() {
    ...
    // Load user data here to ensure provider is ready
  ...
  Future<void> _saveProfile() async {
    ...
    try {
      // Try to upload profile picture if selected (but don't block if it fails)
      if (_selectedImage != null) {
        try {
          final imageResponse = await ProfileService.uploadProfilePicture(_selectedImage!);
          if (imageResponse['success'] != true) {
            // Show warning but continue with profile update
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text('Note: ${imageResponse['message'] ?? 'Image upload failed, but profile will be updated'}'),
      ...
      // Update profile data (even if image upload failed)
